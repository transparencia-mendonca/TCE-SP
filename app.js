...
function diaryTriageGroups(){
  ...
  const out=[];let delegatedExcluded=0,cohortExcluded=0;
  for(const g of buckets.values()){
...
function renderTravel(){
  const groups=diaryTriageGroups(),delegatedExcluded=groups.delegatedExcluded||0,cohortExcluded=groups.cohortExcluded||0;
...
if("serviceWorker"in navigator)navigator.serviceWorker.register("sw.js?v=0913",{updateViaCache:"none"});let deferred;window.addEventListener("beforeinstallprompt",e=>{e.preventDefault();deferred=e;$("#installBtn").hidden=false});$("#installBtn").onclick=async()=>{if(deferred){deferred.prompt();deferred=null}};
